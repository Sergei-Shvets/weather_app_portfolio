...
const Search = ({term, options, onInputChange, onOptionSelect, onSubmit}: Props): JSX.Element => {
  ...
  return (
    <main className="flex justify-center items-center h-[100vh] w-full">
      ...
      h-full lg:h-[500px] bg-white bg-opacity-20 backdrop-blur-lg drop-shadow-lg rounded text-zinc-700">
        ...
        <div className="relative flex mt-10 md:mt-4">
          ...
          <ul className="absolute top-9 bg-white ml-1 rounded-b-md">
            {options.map((option: optionType, index: number) => (
              <li key={option.name + '-' + index}>
                <button className="text-left text-sm w-full px-2 py-1 hover:bg-zinc-700 hover:text-white"
          ...
          <button className="rounded-r-md border-2 border-zinc-100 hover:border-zinc-500 hover:text-zinc-500 text-zinc-100
